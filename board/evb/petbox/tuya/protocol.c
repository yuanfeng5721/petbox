...
{
  {DPID_FEED_NUM, DP_TYPE_VALUE},
  {DPID_FEED_WEIGHT, DP_TYPE_VALUE},
  {DPID_HISTORY_DATA, DP_TYPE_BOOL},
  {DPID_FEED_VOICE_RECORD, DP_TYPE_ENUM},
...
void uart_transmit_output(unsigned char value)
{
	...
	extern int at_uart_send_data(const unsigned char *data, unsigned short size);
    at_uart_send_data(&value, 1);
/*
    //Example:
...
//    mcu_dp_value_update(DPID_FEED_NUM,当前手动喂食); //VALUE型数据上报;
//    mcu_dp_value_update(DPID_FEED_WEIGHT,当前单份食物重量); //VALUE型数据上报;
//    mcu_dp_bool_update(DPID_HISTORY_DATA,当前投喂记录); //BOOL型数据上报;
//    mcu_dp_enum_update(DPID_FEED_VOICE_RECORD,当前短音录制); //枚举型数据上报;
//    mcu_dp_enum_update(DPID_REALTIME_DATA,当前实时数据); //枚举型数据上报;
//    mcu_dp_bool_update(DPID_WATER_NUM,当前手动喂水); //BOOL型数据上报;
//    mcu_dp_value_update(DPID_FEED_REMAINING,当前余粮显示); //VALUE型数据上报;
}
...
******************************************************************************/
/*****************************************************************************
函数名称 : dp_download_feed_num_handle
功能描述 : 针对DPID_FEED_NUM的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_feed_num_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long feed_num;
    
    feed_num = mcu_get_dp_download_value(value,length);
    /*
    //VALUE类型数据处理
...
/*****************************************************************************
函数名称 : dp_download_history_data_handle
功能描述 : 针对DPID_HISTORY_DATA的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_history_data_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    unsigned char history_data;
    
    history_data = mcu_get_dp_download_bool(value,length);
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_HISTORY_DATA,history_data);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_feed_voice_record_handle
功能描述 : 针对DPID_FEED_VOICE_RECORD的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_feed_voice_record_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为ENUM
    unsigned char ret;
    unsigned char feed_voice_record;
...
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_water_num_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    unsigned char water_num;
    
    water_num = mcu_get_dp_download_bool(value,length);
	LOG_I("water_num = %d\r\n", water_num);
    if(water_num == 0) {
        //开关关
    }else {
...
void wifi_test_result(unsigned char result,unsigned char rssi)
{
    ...
    if(result == 0) {
        ...
        if(rssi == 0x00) {
			...
			LOG_E("don't scan wifi ap!!!!!\r\n");
        }else if(rssi == 0x01) {
            //模块未授权
			LOG_E("module don't authorization!!!!!\r\n");
        }
    }else {
        //测试成功
        //rssi为信号强度(0-100, 0信号最差，100信号最强)
		LOG_I("wifi ok, rssi = %d\r\n", rssi);
    }
}
#endif
