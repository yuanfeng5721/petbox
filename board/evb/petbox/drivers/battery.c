...
#define LEVEL_ITEMS 11
static T_BATTERY_CAP_MAP battery_cap_map[LEVEL_ITEMS] = {
	{3300 , 0},  //0%
	{3400 , 10}, //10%
	{3450 , 20}, //20%
	{3500 , 30}, //30%
...
//            APP_PRINT_INFO4("[ADC] adc_sample_demo: ADC one shot mode sample data_%-4d = %d, voltage_%-4d = %dmV ", i, sample_data[i], i, (uint32_t)sample_voltage[i]);
	...
	battery_percent = battery_calc_percent(battery_voltage*2);
...
void battery_timer_callback(void *p_handle)
{
    ...
    if (timer_id == CUSTOM_BATTERY_TIMER_ID)
    {
		ADC_Cmd(ADC, ADC_One_Shot_Mode, ENABLE);
        adc_sample_demo();
		MAKE_CUSTOM_MSG_PARAM(msg, CUSTOM_MSG_BATTERY, CUSTOM_MSG_BATTERY_CAP, battery_percent);
		app_send_msg(msg);
    }
}

#define INTERVAL_MS 1000*10

// Timer to be created.
int battery_timer_init(void)
{
    if (os_timer_create(&battery_handle, "battery", CUSTOM_BATTERY_TIMER_ID,
