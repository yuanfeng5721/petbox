 ...
 *        -1004 : no memory
 */
int at_obj_wait_connect(uint32_t timeout)
{
    int result   = QCLOUD_RET_SUCCESS;
    at_response_t resp = NULL;
	...
	start_time = HAL_GetTimeMs();
	while(1)
	{
		/* Check whether it is timeout */
        if (HAL_GetTimeMs() - start_time > timeout)
        {
            Log_e("wait AT client connect timeout(%d).", timeout);
            result = QCLOUD_ERR_TIMEOUT;
            break;
        }
		...
		if (QCLOUD_RET_SUCCESS != result) {
			Log_e("at client connect fail");
		} else {
			Log_i("at client connect success");
			break;
		}
	}
