...
int cmd_uart_vsprintf(char *buf, const char *fmt, const int *dp)
{
    char *p, *s;
    ...
    for (; *fmt != '\0'; ++fmt)
    {
        if (*fmt != '%')
        {
            buf ? *s++ = *fmt : cmd_uart_send_char(*fmt);
            continue;
        }
        if (*++fmt == 's')
        {
 ...
 * \endcode
 */
void cmd_uart_init(void *event_queue_handle, void *io_queue_handle)
{
